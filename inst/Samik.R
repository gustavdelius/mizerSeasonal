...
sp$rdd_vonMises_kappa <- c(3.6047, 2.9994, 1.944, 1.141, 0.40493, 0.795, 4.973,1.4257, 4.0495, 3.6567, 5.4732, 1.951)[order]
...
plotGonadsVsTime(sim2, sizes = 0.8 * sim2@params@species_params$w_max)
    ...
    feed_bit[, i] <- 
        sapply(1:(Times * 10 + 1),
               function(t) {
                   ...
                   getEncounter(ps, n = sim@n[t, , ],
                                n_pp = sim@n_pp[t, ],
                                n_other = n_other)[i, w_max_idx]
                   }
               )
}

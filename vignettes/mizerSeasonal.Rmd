...
We see that the system has settled down to a steady state quite quickly. To see the variation within the year we project for a further year but with higher time resolution
```{r}
...
Because we are in the steady state, the next year will again look the same, so you can simply play the above simulation repeatedly.
